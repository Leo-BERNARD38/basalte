...
function newPage(): readonly string[] {
  return [
    ...
    '2. Écris `meta.title` et `meta.description` dans **chaque** langue en',
    '   ligne : les deux sont requis, et `npm run check` refuse la page sans',
    '   eux. Une langue en préparation peut rester vide.',
    ...
    '5. `npx basalte content` dit ce que les autres pages portent déjà. Un',
    '   titre ou une description repris de l’une d’elles est signalé par',
    '   `npm run check` : les moteurs n’en afficheraient qu’une.',
    '',
    '## Toujours finir par',
    '',
...
function content(): readonly string[] {
  return [
    ...
    '',
    '`npx basalte content` relève ce que le site porte déjà : les pages, leurs',
    'sections, l’avancement des traductions, les médias qu’aucune section',
    'n’emploie. Le lire évite d’ouvrir les JSON un par un — et de reprendre un',
    'titre déjà pris.',
    '',
    '## Toujours finir par',
    '',
    '`npm run check`. Il refuse un champ requis vide — titre et description de',
    'page compris —, un texte trop long, une traduction manquante dans une',
    'langue en ligne et une image absente ou sans texte alternatif.',
  ]
}
