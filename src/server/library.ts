...
//
// Une requête qui n’annonce pas sa longueur est refusée de la même façon : un
// navigateur l’annonce toujours, et l’omettre est le moyen de faire lire sans
// limite. Les octets réels restent vérifiés par `ingest`, qui a le dernier mot.
...
export async function uploadMedia(
  ...
  if (!withinLength(request, MAX_IMAGE_BYTES + ENVELOPE_SLACK)) {
    return json(
      {
        ok: false,
        message: `Image trop lourde : la limite est de ${MAX_IMAGE_BYTES / 1024 / 1024} Mo.`,
      },
      413,
    )
  }
