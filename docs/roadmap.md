...
| **Un bloc `carousel`** — une piste qui défile en `scroll-snap`, sans une ligne de script : ce serait la démonstration la plus directe d'une liste qu'aucune borne ne protège (D160) | Le premier client qui a plus d'images à montrer qu'une page n'en porte. `gallery` montre déjà plusieurs images qui s'allongent sans casser, et un dix-septième bloc de référence doit apporter une mécanique que les seize autres ne montrent pas (D148) |
...
| **La mise en page des seize blocs de `src/blocks`** — la phase 18 a refait la direction artistique du panel, pas celle des blocs, qui consomment les tokens du **site** : leur rythme vertical, leurs largeurs de lecture et leurs variantes bureau n'ont jamais été repris ensemble (deux blocs sur seize portent un `.desktop.astro`) | Le premier client dont la maquette est refusée pour la mise en page d'un bloc du socle plutôt que pour ses tokens. La règle des tokens tient déjà l'essentiel : un bloc qui déplaît se corrige aujourd'hui par `site.config.ts`, sans toucher au socle |
...
| **Un texte différent entre bureau et mobile**, et une section visible sur un seul support — dessinés dans la maquette de la phase 18, non retenus | Un client qui montre une page où le même texte ne peut pas tenir les deux largeurs, **et pour qui raccourcir le texte des deux n'est pas la réponse**. D107 refuse l'axe de support sur `hidden` avec une raison intacte — « le mobile cesserait de porter tout le contenu, ou le bureau serait amputé sans que le panel sache le dire » — et `checkRenders` compare les deux rendus après chaque build (D108) : ouvrir cette vanne demande de décider d'abord ce que devient ce contrat |
