...
| **La pagination de l'index du journal** — il porte tous les billets, groupés par année, et rien ne le découpe | Le premier journal qui dépasse deux cents billets, ou dont la page d'index sort du budget de poids ci-dessus. Découper demande des routes, des canoniques et des `prev`/`next` : c'est un travail, pas un réglage |
